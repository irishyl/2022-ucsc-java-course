...
public class TestIO {
	...
	static void testSystemIn() throws IOException { // what happens if you don't throw IOException?
		...
		// System.out.println((char) b); //because it is a byte stream, and reads
		// integer so not much of use to us
		 ...
		 * System.out.println("Byte read: " + (char)byteArray[0] + (char)byteArray[1] +
		 * (char)byteArray[2] + (char)byteArray[3]);
		 */ }

	//note working with File
	...
	//note Scanner can be used with the disk File as well
	...
	static void testFileReaderWithBufferedReader(String inputDiskFileName) {
		try {
			...
			// shorter version:
			// BufferedReader finalInStream = new BufferedReader(new
	...
	static void testTokenizer(String fileName) {
		...
		try {
			...
			// this code executes if there is an error in getting the next token from the file
			System.out.println(err);
		} // catch
	}

	//note StringTokenizer Example I
	...
	//note StringTokenizer Example II using Console.
	...
	public static void main(String[] args) throws IOException {
		...
		testFileReaderDirectly(args.length > 0 ? args[0] : "input_final.txt"); // read one byte at a time
																				// testFileReaderWithBufferedReader("input_final.txt");
																				// //read while line
